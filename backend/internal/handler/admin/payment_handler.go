...
// GetOrderStats returns admin order profitability statistics.
// GET /api/v1/admin/payment/order-stats
func (h *PaymentHandler) GetOrderStats(c *gin.Context) {
	params, ok := parseAdminOrderListParams(c)
	if !ok {
		return
	}
	stats, err := h.paymentService.GetAdminOrderStats(c.Request.Context(), params)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
