...
func TestGetDashboardStatsIncludesProfitStatsForWindow(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	user := createPaymentStatsUser(t, ctx, client, "dashboard-profit@example.com")
	svc := &PaymentService{entClient: client}

	now := time.Now()
	createPaymentStatsOrder(t, ctx, client, paymentStatsOrderInput{
		UserID: user.ID, UserEmail: user.Email, UserName: user.Username,
		OutTradeNo: "dashboard-inside", CreatedAt: now.AddDate(0, 0, -2),
		Status: OrderStatusCompleted, Amount: 100, PayAmount: 105, FeeRate: 2,
	})
	createPaymentStatsOrder(t, ctx, client, paymentStatsOrderInput{
		UserID: user.ID, UserEmail: user.Email, UserName: user.Username,
		OutTradeNo: "dashboard-refund", CreatedAt: now.AddDate(0, 0, -1),
		Status: OrderStatusRefunded, Amount: 40, PayAmount: 42, RefundAmount: 42,
	})
	createPaymentStatsOrder(t, ctx, client, paymentStatsOrderInput{
		UserID: user.ID, UserEmail: user.Email, UserName: user.Username,
		OutTradeNo: "dashboard-outside", CreatedAt: now.AddDate(0, 0, -40),
		Status: OrderStatusCompleted, Amount: 999, PayAmount: 999,
	})

	stats, err := svc.GetDashboardStats(ctx, 30)

	require.NoError(t, err)
	require.NotNil(t, stats.ProfitStats)
	require.Equal(t, 2, stats.ProfitStats.TotalOrders)
	require.InDelta(t, 147, stats.ProfitStats.GrossPayAmount, 0.001)
	require.InDelta(t, 42, stats.ProfitStats.RefundAmount, 0.001)
	require.InDelta(t, 102.9, stats.ProfitStats.NetPayAmount, 0.001)
}

func TestGetDashboardStatsForRangeUsesCustomWindow(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	user := createPaymentStatsUser(t, ctx, client, "dashboard-range@example.com")
	svc := &PaymentService{entClient: client}
	...
	createPaymentStatsOrder(t, ctx, client, paymentStatsOrderInput{
		UserID: user.ID, UserEmail: user.Email, UserName: user.Username,
		OutTradeNo: "dashboard-range-before", CreatedAt: start.Add(-2 * time.Hour),
		Status: OrderStatusCompleted, Amount: 100, PayAmount: 100,
	})
	createPaymentStatsOrder(t, ctx, client, paymentStatsOrderInput{
		UserID: user.ID, UserEmail: user.Email, UserName: user.Username,
		OutTradeNo: "dashboard-range-inside-1", CreatedAt: start.Add(2 * time.Hour),
		Status: OrderStatusCompleted, Amount: 20, PayAmount: 21, FeeRate: 1,
	})
	createPaymentStatsOrder(t, ctx, client, paymentStatsOrderInput{
		UserID: user.ID, UserEmail: user.Email, UserName: user.Username,
		OutTradeNo: "dashboard-range-inside-2", CreatedAt: start.AddDate(0, 0, 2).Add(3 * time.Hour),
		Status: OrderStatusRefunded, Amount: 40, PayAmount: 42, RefundAmount: 42,
	})
	createPaymentStatsOrder(t, ctx, client, paymentStatsOrderInput{
		UserID: user.ID, UserEmail: user.Email, UserName: user.Username,
		OutTradeNo: "dashboard-range-after", CreatedAt: end.Add(time.Hour),
		Status: OrderStatusCompleted, Amount: 200, PayAmount: 200,
	})

	stats, err := svc.GetDashboardStatsForRange(ctx, start, end)

	require.NoError(t, err)
	require.Equal(t, 1, stats.TotalCount)
	require.InDelta(t, 21, stats.TotalAmount, 0.001)
	require.Len(t, stats.DailySeries, 3)
	require.Equal(t, "2026-07-10", stats.DailySeries[0].Date)
	require.InDelta(t, 21, stats.DailySeries[0].Amount, 0.001)
	require.Equal(t, "2026-07-12", stats.DailySeries[2].Date)
	require.InDelta(t, 0, stats.DailySeries[2].Amount, 0.001)
	require.NotNil(t, stats.ProfitStats)
	require.Equal(t, 2, stats.ProfitStats.TotalOrders)
	require.InDelta(t, 63, stats.ProfitStats.GrossPayAmount, 0.001)
	require.InDelta(t, 42, stats.ProfitStats.RefundAmount, 0.001)
}
